...
#define DDImage_Op_ABI kDDImage_ABI_version
    ...
    public:
      ...
      //! A single Op input containing the location of an upstream Op, the state for the Op at
      //! this Op's input connection point, and an upstream Node if one exists.
      //!
      //! A valid Input must have a non-null \a op pointer, while \a node can be null. A null
      //! \a node will commonly occur when Ops are directly connected together without creator
      //! Nodes.
      //!
      //! Note that if non-null \a node may not be the same Node that \a op was generated by
      //! since there may be additional non-OpNodes in between \a op's Node and this Op's Node.
      //! When this is the case \a graphState may be different than the one stored in \a op
      //! due to those additional Nodes modifying the graph info.
      struct Input : public OpGraphLocation {
        Input() = default;

        //! Op ctor takes the graph state information from the Op. Warning - the graph topology
        //! for this Op must not contain hidden modifier Nodes, otherwise the graph state info
        //! in the Op may not be accurate. Use Op::inputConnection() to retrieve the full graph
        //! context.
        explicit Input(Op*);

        //! Construct from a graph location also setting the Op input index number (note - not
        //! the Node input index number, they can be different if Op is split.)
        ...
        explicit operator bool() const { return (op != nullptr && _index >= 0); }
      ...
      protected:
        friend class Op;
        int32_t _index = -1;  //!< Index number of the input in the Op's inputs() list
      };
...
#error "Remove the above code block in Nuke 18 as they are deprecated in Nuke 17."
#endif
      InputList _inputs;      //!< input(0)..input(n)
      OpList    _children;    //!< Stores ops that set their parent to this op
      OutputSet _outputs;     //!< Stores ops that use this as an input, and the input numbers they use.

      //! Whether the ops pointed to by _inputs are actually for the right contexts
      bool _inputsValid;
      ...
      bool recycling_; //! True if the op is currently being recycled
    ...
    protected:
      ...
      Hash _editVersionHash; //!< Op's current edit version state, usually the combination of input Op & knob edit version states
      ...
      virtual void _knobStorePost(const OutputContext& opStoreContext,
                                  Hash&                newOpHash,
                                  Hash&                newOpEditVersionHash) { }
      ...
      //! Allow derived op classes to react when an op is recycled for a different output
      //! context. This is typically used to perform additional re-initialisation for the
      //! new context, which may require the affected objects to be locked until the op has
      //! been fully recycled.
    ...
    public:
      ...
      //! Op's global edit version state hash, usually formed from the input and local knobs
      //! edit version hashes.
      //! This version hash should only change when an Op's -global- state is changed, not its
      //! OutputContext(time & view) sampled state which is communicated by the Op::hash().
      //! Typically the global version state changes only when the contents of a Knob is
      //! changed (i.e. edited) or the graph topology is changed (i.e. input connections
      //! edited.)
      //! Op::treeVersion() provides a similar global change notification except that it is
      //! global to -all- of Nuke, so Op::editVersionHash() allows the individual Node/Op
      ...
      //! repeat even when Knobs are set back to a previously used value. Op::hash() should
      //! exhibit that behavior, not Op::editVersionHash()!
      //!
      //! Default implementation returns the local _editVersionHash built automatically
      //! during Op construction, but a subclass can override this to include additional
      //! parameters.
      ...
      //! state implement Op::editVersionHash() instead.
      //! Default implementation sets the local _editVersionHash to the new hash value.
      ...
      //! Returns the Node identifier name, which can be different than Op::Class() when
      //! an Op::Description declares it wants to be owned by a custom Node owner class.
      //! Usually the node identifier name used in the Op::Description should match this
      //! method's returned name, but the Op can return whatever it wants as a Node
      //! identifier name.
      //! Default implementation returns NodeI::nodeDescriptionName().
      virtual const char* nodeClass() const;

      //! Returns the Node name. Warning, this can be null!
      //! Default implementation returns NodeI::name() which is the Node's local name with
      //! no parent path.
      ...
      */
      ...
      /*! Nuke calls this method just *after* knobs() is called in Op store mode
         ...
         The output Op::hash() and Op::editState() can be manipulated at the same
         time, reducing the need to repeat calculations in Op::append(). The
         incoming hash variables contain the result of the hashes built in knobs()
         plus possibly input hash contributions. See Op::append() for more info.
      ...
      */
      void knobStorePost(const OutputContext& opStoreContext,
                         Hash&                newOpHash,
                         Hash&                newOpEditVersionHash);
      ...
      */
       ...
       */
      int inputs() const { return static_cast<int>(_inputs.size()); }
      ...
      /*! Retrieve or create a version of Class \a baseOp from the owner Node at a specific
          OutputContext. Note that the returned Op* may be the same as \a baseOp if the
          OutputContexts match and \a baseOp is fully constructed. That is, do not assume
          that this method will always allocate a new Op.
      */
      ...
      */
      ...
      /*! Retrieve or create an Op from the node input at a specific OutputContext. This
          is the same as calling Op::node_input(input, type, &context).
      */
      ...
      //! Retrieve the Input for \a input index. Same as getInputConnections()[input]
      //! but with range checking.
      const Input& inputConnection(int32_t input) const;

      //! Retrieve the 'flat' Input for \a base index and \a offset with
      //! range checking. See inputNumber() for more info.
      ...
      mFnDeprecatedInNuke17("getInputs() has been deprecated, use getInputOps() instead")
      const OpList& getInputs() const { return _allInputs; }
#else
#error "Remove the above code block in Nuke 18 as they are deprecated in Nuke 17."
#endif

      //! Return a vector of all input connection Ops, including input0 and other inputs.
      //! Note that this is built on demand from \a _inputs, so a cheaper/faster method is
      //! to use getInputConnections() and retrieve the Op* from each Input in that list.
      OpList getInputOps() const;

      //! Return a reference to a list of all Input connections, including input0 and
      //! \a other_inputs. Note that this may contain null pointers if some in-between
      //! inputs are disconnected.
      const InputList& getInputConnections() const { return _inputs; }

      //! Returns a reference to a vector of all child ops.
      //! Avoid using this; it is only really public for op tree traversal.
      const OpList& getChildren() const { return _children; }
       ...
       */
      mFnDeprecatedInNuke17("set_input(int, Op*, int, int) has been deprecated, use setInput(int, Input) and implement _setInput() instead")
...
#error "Remove the above code block in Nuke 18 as they are deprecated in Nuke 17."
      ...
      void set_input0(Op* op) { setInput(0, Input(op), 0, 0); }
      void set_input(int i, Op* op) { setInput(i, Input(op), i, 0); }
      void set_input(int i, Op& op) { setInput(i, Input(&op), i, 0); }

      //! Same as setInput(inputNumber(n, offset), op, n, offset);
      void set_input(int input, int offset, Op* op) { setInput(inputNumber(input, offset), Input(op), input, offset); }

      //! Same as setInput(input, connection, input, 0).
      void setInput(int32_t input, const OpGraphLocation& connection) { setInput(input, connection, input, 0); }

      //! Set input connection, auto-expanding input list size to accomodate \a input.
      //! 
      //! An Op subclass can perform its own custom work by implementing the virtual
      //! _setInput() which is called from inside this method -before- this Op's
      //! input is set. The arguments to inputNumber() are also provided to assist
      //! with this, where \a base is the non-split (node) input number, and
      //! \a offset is the split index.
      ...
      /*! \fn void Op::setRecycling(bool)
          Used to indicate that the op is being recycled.
          This should be set to prevent unused ops that are in the
          process of being recycled from being mistakenly recycled
          again when recursively regenerating their inputs. 
          An overrideable _setRecycling method is provided for
          derived classes to implement additional recycling behaviour.
       */
      void setRecycling(bool recycling);

      /*! \fn bool Op::recycling()
          Indicates whether the op is in the process of being recycled.
       */
      bool recycling() const { return recycling_; }
    ...
    public:
      ...
      //! Returns the asset's local file path in the resolvedAssetPath parameter, if it has one.
      //! Returns false if it failed to resolve an asset
      ...
      public:
        const char* name;     //!< Nuke script command name
        const char* nodeBase; //!< Base Node (OpNode subclass) name
        ...
        Description(const char* n, OpConstructor c, License* lic = nullptr) :
          constructor_(c), name(n), nodeBase(nullptr)
        {
          license = lic;
          ctor(add);
        }
        ...
        //! Same but with a separate base Node (OpNode subclass) class to create for
        //! this Op type. In this mode the Nuke script command name remains the Op's
        //! name (Grade, Blur, etc) but allows the constructed Op to be a child of a
        //! specific Node class.
        //!
        //! If the Node class is not found a warning is output and the Node
        //! constructor falls back to creating a generic OpNode.
        Description(const char* opClassName, OpConstructor c, const char* nodeBaseName, License* lic = nullptr) :
          constructor_(c), name(opClassName), nodeBase(nodeBaseName)
        {
          license = lic;
          ctor(add);
        }
...
#if kDDImageVersionMajorNum <= 17
        mFnDeprecatedInNuke17("Description(menu, OpConstructor) has been deprecated, use Description(OpConstructor) instead")
...
#error "Remove the above code block in Nuke 18 as they are deprecated in Nuke 17."
      ...
      /*! This function prints a formated message to stdout. If DDImage runs
          ...
          - 'x' will show a question with a cancel option, and will return 1 for yes, 0 for no, -1 for cancel.
       */
    ...
    template<> inline AxisSceneOp* op_cast<AxisSceneOp*>( Op* op ) { return op ? op->axisSceneOp() : nullptr; }
    template<> inline CameraOp* op_cast<CameraOp*>( Op* op ) { return op ? op->cameraOp() : nullptr; }
    template<> inline CameraSceneOp* op_cast<CameraSceneOp*>( Op* op ) { return op ? op->cameraSceneOp() : nullptr; }
    template<> inline LightOp* op_cast<LightOp*>( Op* op ) { return op ? op->lightOp() : nullptr; }
    template<> inline LightSceneOp* op_cast<LightSceneOp*>( Op* op ) { return op ? op->lightSceneOp() : nullptr; }
    template<> inline SceneOpI* op_cast<SceneOpI*>( Op* op ) { return op ? op->sceneOpI() : nullptr; }
    template<> inline GeometryProviderI* op_cast<GeometryProviderI*>( Op* op ) { return op ? op->geometryProvider() : nullptr; }

    DDImage_API
    std::ostream& operator << (std::ostream&, const Op::Input&);
