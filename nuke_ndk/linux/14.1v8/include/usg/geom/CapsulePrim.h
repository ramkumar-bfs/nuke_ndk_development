...
#ifndef USG_GEOM_CAPSULEPRIM_H
#define USG_GEOM_CAPSULEPRIM_H
...
namespace usg {
  ...
  class USG_API CapsulePrim : public GprimPrim {
  public:
    CapsulePrim();
    CapsulePrim(const Prim& prim);
    ~CapsulePrim();

    bool validate();
    static bool isValidPrim(const Prim& prim);
    static CapsulePrim defineInLayer(const LayerRef& layer, const Path& path);
    static CapsulePrim overrideInLayer(const LayerRef& layer, const Prim& prim);
    static CapsulePrim getInStage(const StageRef& stage, const Path& path);
