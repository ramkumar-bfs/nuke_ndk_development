  ...
  */
  ...
  public:
    ...
    */
    ...
    /*! Return a newly composed stage from this engine's -current- root layer sublayer
        stack (ie the sublayer stack currently in \a _rootLayer.) This is a convenience
        for engines that need an intermediate composed stage that prims can be queried
        on without affecting the stage contained in \a context passing down the graph.

        Note that composing a stage can be expensive so only call this when absolutely
        necessary.

        An engine can override this method to provide a stage from an alternate source.
    */
    virtual usg::StageRef  composedStage(bool flatten = false);

    /*! If the input is a GeomEngine return a filled in, composed Stage from
        it. The returned StageRef is not cached in the engine so the calling
        method must copy it local or the bound Stage may go out of scope.

        Default implementation calls input->buildScenegraph() with a duplicated
        GeomSceneContext containing a new, empty input Stage so the existing stage
        in \a context is left unaffected.
        The constructed input Stage is then returned.

        An engine could change this behavior to return a Stage from an alternate
        source.
    */
    ...
    */
    void          buildScenegraph(GeomSceneContext& sceneCtx,
    ...
    */
    ...
    /*! Return the optional output Stage cache stored on this engine. 
        This stage is for external use and not locally managed by the engine, which is
        just providing a stable location to hold the cache.
    */
    virtual usg::StageRef&        outputStageCache() { return _outputStageCache; }
    virtual const usg::StageRef&  outputStageCache() const { return _outputStageCache; }
  ...
  protected:
    ...
    */
    ...
    /*! GeomEngine subclass can implement to return a root layer for a specific
        engine input.
        Does not call buildScenegraph()!
    */
    virtual usg::LayerRef rootLayerFromInput(int32_t input = 0);
  ...
  protected:
    ...
    fdk::StringList  _stageSubLayers;       //!< List of sublayers last set in stage's rootLayer
    fdk::Hash        _stageSubLayersHash;   //!< Hash of _stageSubLayers used to quickly check rootlayer state change
    ...
    //----------------------------------------------------------------------
    // Optional output caches
    usg::StageRef         _outputStageCache;  //!< Optional output Stage cache storage for external use (not locally managed)
