...
#ifndef USG_LUX_DISKLIGHTPRIM_H
#define USG_LUX_DISKLIGHTPRIM_H
...
namespace lux {
  ...
  class USG_API DiskLightPrim : public BoundableLightBasePrim {
  public:
    DiskLightPrim();
    DiskLightPrim(const Prim& prim);
    ~DiskLightPrim();

    bool validate();
    static bool isValidPrim(const Prim& prim);
    static DiskLightPrim defineInLayer(const LayerRef& layer, const Path& path);
    static DiskLightPrim overrideInLayer(const LayerRef& layer, const Prim& prim);
    static DiskLightPrim getInStage(const StageRef& stage, const Path& path);
