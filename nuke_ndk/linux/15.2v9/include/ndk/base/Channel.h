...
namespace ndk {
  ...
  public:
    ...
    enum Type : int32_t
    {
      Auto = -1,      //!< Special value: Determine type from channel/layer name
      None = 0,       //!< Generic value: ordering=unimportant, blendable=unknown, interpolatable=unknown
      //
      // -------- Blendable -------
    ...
    static Type         TypeCompoundType(Type type);
    ...
    //! Note this method returns an int to avoid including the usg headers, so casting to
    //! usg::Value::Type may be required.
    static int32_t      TypeCompoundValueType(Type type);


  public:
    //! Default ctor leaves junk in channel
    Channel() {}
    Channel(const Channel& b) : _index(b._index) {}
