...
#ifndef USG_GEOM_CONEPRIM_H
#define USG_GEOM_CONEPRIM_H
...
namespace usg {
  ...
  class USG_API ConePrim : public GprimPrim {
  public:
    ConePrim();
    ConePrim(const Prim& prim);
    ~ConePrim();

    bool validate();
    static bool isValidPrim(const Prim& prim);
    static ConePrim defineInLayer(const LayerRef& layer, const Path& path);
    static ConePrim overrideInLayer(const LayerRef& layer, const Prim& prim);
    static ConePrim getInStage(const StageRef& stage, const Path& path);
