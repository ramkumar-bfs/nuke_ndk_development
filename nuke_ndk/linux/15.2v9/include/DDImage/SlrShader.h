...
DD::Image::ChannelSet getInputIntArrayValueAsChannelSet(const usg::ShaderDesc&       shader_desc,
  ...
  */
  class DDImage_API SlrShader : public SlrPlugin
  {
  public:
    ...
    */
    ...
    struct InputBinding
    {
      ...
      inline bool haveRgb() const { return (port) ? port->haveRgb : false; }

      //! Returns true if the bound output port provides alpha.
      inline bool haveAlpha() const { return (port) ? port->haveAlpha : false; }
    };
    ...
    struct Color4Binding : public InputBinding
    {
      ...
      inline bool haveAlpha() const { return (opacityPort) ? opacityPort->haveAlpha : false; }
  ...
  public:
    ...
    */
    ...
    public:
      ...
      PluginDescription(const char*   shaderClass,
      ...
      */
      ...
      const char*         shaderClass() const { return _pluginClass; }
    };
    //-----------------------------------------------------------------
  ...
  protected:
    SlrRenderContext*      _slrtx;                  //!< Renderer owner
    ...
    //
    ShadingLayerReqSet     _requestedInputPrimvars; //!< Input primvar attributes being requested
    ...
    static SlrShader* _create(const char* shader_class);
  ...
  public:
    ...
    */
    ...
    static SlrShader*  create(const char* shader_class);
  ...
  public:
    ...
    //! Is shader operating in 'legacy' mode? ie using the old ScanlineRender1 architecture?
    //! In legacy mode certain features such as dynamic primvar retrieval are not supported, so a shader
    //! can test this before deciding to request primar channels it expects to be filled in. For example
    //! a shader that wants to sample additional custom texture primvars will not be able to because
    //! the legacy architecture only support a single hardcoded UV value, so the shader could raise an
    //! error or warning to the renderer, or fallback to a default output result.
  ...
  public:
    ...
    */
    ...
    /*! Request the resources this shader will want to use during shading. For
        ...
        The shader should request an Iop sampler by calling slrtx.requestTextureSampler()
    ...
    */
    ...
    /*! Initialize the shader's state in preparation for shading.

        At this point any Iop texture samplers requested in requestResources() have
        been created by the Renderer and the shader can make local copies of the
        ...
        _requestedInputPrimvars set) are now assigned Channel indices in shading channel
        manager and can be retrieved via SlrRenderContext::retrievePrimvarLayer(<primvar_name>)
    ...
    */
    ...
    /*! Return the set of named primvars this shader wants to read from the geometry.
        ...
        via SlrRenderContext::retrievePrimvarLayer(<primvar_name>). Note that <primvar_name>
        is the name of the primitive attribute -without- the leading 'primvars:' namespace.
    ...
    */
    virtual const ShadingLayerReqSet&  requestedInputPrimvars() const;
    ...
    */
    ...
    /*! Return the total set of primvars this shader is requesting from the geometry,
        ...
        value up to date.
    */
    const ShadingLayerReqSet&      inputPrimvars() const { return _requestedInputPrimvars; }
