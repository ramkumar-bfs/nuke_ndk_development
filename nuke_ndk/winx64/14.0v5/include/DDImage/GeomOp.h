...
namespace DD {
  namespace Image {
    ...
    /*! For passing knob value snapshots between GeomOp and GeomOpEngine.
        ...
        TODO: Each GeomKnobSample needs to remember what view it's configured
    ...
    */
    ...
    public:
      ...
      using OpInput            = std::pair<int32_t, Op*>;
      ...
      */
      ...
      public:
        ...
        */
        const OpInputTimeMap&  opsForView(int32_t view) const;
    ...
    public:
      ...
      */
      ...
      //! Convenience method to get a value from a Knob, if it exists.
      template<typename T>
      ...
      //! Convenience method to get a value from a Knob, if it exists. If knob is null nothing is done.
      template<typename T>
      T     getValue(const T&    defaultValue,
      ...
      */
      const InputBinding&  inputBinding(uint32_t node_input);
      ...
      */
      GeomEngine*  inputEngine(uint32_t node_input) override { return inputBinding(node_input).geomEngine(); }
      GeomEngine*  inputEngine0()                   override { return inputBinding(0).geomEngine(); }
      GeomEngine*  inputEngine1()                   override { return inputBinding(1).geomEngine(); }


      /*! Returns the matching Op input for a specific frame and view, if one exists,
          from the node_input InputBinding.
          The frame number is rounded to three decimal places (ie .001, a thousandth of a frame)
          to make it more likely that a match will occur.
          If frame is the 'default' frame value this returns the first Op in the list,
          usually the earliest in time.
          If view is -1 (invalid) then the 'main' view (view index 1) is used instead.

          If \a generate_op is true then this method will dynamically-create the Op for
          the requested time.
      */
      Op*  inputOpAt(uint32_t node_input,
                     double   frame,
                     int32_t  view = -1,
                     bool     generate_op = false);
      //! Same as inputOpA() but with view set to use the default view.
      Op*  inputOpAt(uint32_t node_input,
                     double   frame,
                     bool     generate_op = false) { return inputOpAt(node_input, frame, -1, generate_op); }
      //! Same as inputOpAt() but with OutputContext for frame/view.
      Op*  inputOpAtContext(uint32_t             node_input,
      ...
      */
      ...
      /*! Merges the layers from the list of inputs into the \a _rootLayer.
          These are merged in reverse order.
      */
      void  mergeFromInputs(usg::GeomSceneContext&      context,
                            const std::vector<int32_t>& inputs) override;

      /*! Merges the layers from \a input up to \a input + \a count into the \a _rootLayer.
          These are merged in reverse order.
      */
      ...
      /*! Update the state of the GeomOpEngine in preparation for calling getScenegraph()
      ...
      */
      ...
      /*! The 'request' hash is used to track the evaluation or configuration state
          ...
          The request hash is built in GeomOp::getStage() from the set of GeomOps
          passed to it. If those GeomOps are the same and their hashes haven't
          changed, then the Stage owned by this GeomOpEngine is assumed to be
          unchanged and returned quickly.
      */
      const fdk::Hash&  lastRequestHash() const { return _lastRequestHash; }
      //! Assign the value of the request hash, usually done in GeomOp::getStage().
    ...
    protected:
      ...
      fdk::Hash   _lastRequestHash;   //!< The last combined GeomOp hashes passed to the most recent getStage()
      //
      ...
      /*! Converts the input Op(s) to surface shader and material Prims, defined in the
          edit GeomLayer anchored under \a material_library_path.
      ...
      */
      ...
      /*! Converts the input LightOp(s) to a usg::LightPrim and any associated shaders, defined
          in the edit GeomLayer anchored under \a light_anchor_path.

          If the LightOp has 'surface' shader inputs (texture sources really) they will be
          importShaderOp() 
          will be anchored under 'material_library_path'.
      ...
      */
      virtual usg::Path importLightOp(bool                         create_mode,
                                      ...
                                      const usg::Path&             light_anchor_path,
      ...
      /*! Assign the material to a list of Prims. Companion method to importShaderOp() usually
          ...
          updated.
      */
      virtual void      assignMaterialBinding(bool                  create_mode,
    ...
    /*! \class DD::Image::GeomOp
        ...
        Base class for an Op that produces an usg::Stage containing scenegraph and
        3D geometry data.
        ...
        any of the GeomOps are asked for its output stage the same GeomOpEngine instance
        is processed, the same stage object returned, and the results cached so the next
        GeomOp will produce the same result.

        The scene data is accessed by calling the static GeomOp::getStage() method with
        ...
        A primary advantage of this 'timeless' scheme is that Prims passing through the
        ...
        of their animation data which then becomes available to the downstream consumer
    ...
    */
    ...
    public:
      ...
      /*! Adds the geometry selection knobs. If \a geoselect_saves is false the knobs
          will not write(save) their selection contents to output script.
      */
      virtual void  addSelectionKnobs(Knob_Callback f,
      ...
      /*! Correlate a Knob value with a GeomOpEngine manipulation, specified by the
          ...
          hash of the knob, calculated during Knob::store(), is appended to the hash of
          the named target.

          Call this in the knobs() method right after defining a knob which should affect
          the geometry output in some way.

          For example in the knobs() method of a GeomOp subclass that wants a 'scale'
          parameter to override existing Prim attributes:
            XYZ_knob(f, &params.scale.x, "scale");
              AppendKnobToTarget(f, usg::EngineTokens.target.overrideGeometry);

          The GeomOpEngine class has built-in targets which trigger GeomState flags:
            'define-geometry'  (EngineTokens.target.defineGeometry)
            'modify-values'    (EngineTokens.target.modifyValues)
            'edit-layer'       (EngineTokens.target.editLayer)
            'edit-stage'       (EngineTokens.target.editStage)
            'affect-stereo'    (EngineTokens.target.affectStereo)
          ...
          the results from the most recent knob store(), like the frame, view and hash
          of the knob. This info is used during binding to track the animation state of
          the knob.
      */
      virtual void  AppendKnobToTarget(const Knob_Closure& f,
                                       const usg::Token&   target_id);

      /*! Like AppendKnobToTarget() but for knobs with no current Op OutputContext (ie no
          frame or view.) Intended for global Node knobs like 'name' and 'tile_color'.
      */
      ...
      /*! Use this method if the Knob's value will affect the 3D scenegraph topology by
          ...
          Important note - knobs changes affecting the GeomState::defineGeometryState will
      ...
      */
      ...
      //------------------------------------------------------
      // Build and return a filled in GeomStage
      //------------------------------------------------------
      ...
      */
      ...
      /*! Make changes to the GeomStateTargets registered to this GeomEngine which
          ...
          subclasses should implement the virtual appendGeomState() method and
          append to the local GeomStateTargets via appendToTarget().
      */
      ...
      */
      ...
      // Convenience implementations that affect the 'edit-layer' GeomStateTarget:
      ...
      /*! Return a filled-in, composed GeomStage.
          ...
          If the GeomOp 0 is null, or either GeomOp is invalid an empty GeomStage
          is returned.
          ...
          and views to be processed for the GeomStage. A GeomOpEngine should fill
          ...
          set.
      */
      static usg::StageRef  getStage(const usg::ArgSet&       request_args,
                                     GeomOp*                  geop0,
    ...
    */
    ...
    //
    inline void GeomOp::KnobEditsLayer(const Knob_Closure& f)
      { AppendKnobToTarget(f, usg::EngineTokens.target.editLayer); }
    //
    ...
    //
    inline void GeomOp::appendToEditLayerTarget(const fdk::Hash& value_hash, const fdk::Hash& version, bool is_animating_source)
      { appendToTarget(usg::EngineTokens.target.editLayer, value_hash, version, is_animating_source); }
    inline void GeomOp::appendToEditLayerTarget(const Hash& value_hash, const Hash& version, bool is_animating_source)
      { appendToTarget(usg::EngineTokens.target.editLayer, value_hash, version, is_animating_source); }
    inline void GeomOp::appendToEditLayerTarget(Op* op) { appendToTarget(usg::EngineTokens.target.editLayer, op); }
    //
