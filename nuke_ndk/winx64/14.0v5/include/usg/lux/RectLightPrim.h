...
#ifndef USG_LUX_RECTLIGHTPRIM_H
#define USG_LUX_RECTLIGHTPRIM_H
...
namespace lux {
  ...
  class USG_API RectLightPrim : public BoundableLightBasePrim {
  public:
    RectLightPrim();
    RectLightPrim(const Prim& prim);
    ~RectLightPrim();

    bool validate();
    static bool isValidPrim(const Prim& prim);
    static RectLightPrim defineInLayer(const LayerRef& layer, const Path& path);
    static RectLightPrim overrideInLayer(const LayerRef& layer, const Prim& prim);
    static RectLightPrim getInStage(const StageRef& stage, const Path& path);
